...
public class CapAndHandle : QCAlgorithm
{
    ...
    public override void Initialize()
    {
        ...
        SetCash(100_000); // Starting cash for the algorithm
    ...
    private bool IsItCupAndHandle(List<FinanceCandleStick> candles)
    {
        ...
        // Identify left peak (highest high in first third)
        int leftEnd = count / 3;
        var leftPeakCandle = candles.Take(leftEnd).OrderByDescending(c => c.High).First();
        float leftPeak = leftPeakCandle.High;

        // Identify right peak (highest high in last third)
        int rightStart = 2 * count / 3;
        var rightPeakCandle = candles.Skip(rightStart).OrderByDescending(c => c.High).First();
        float rightPeak = rightPeakCandle.High;

        // Identify bottom (lowest low in middle third)
        var middleCandles = candles.Skip(leftEnd).Take(rightStart - leftEnd).ToList();
        if (middleCandles.Count == 0)
            return false;
        float bottom = middleCandles.Min(c => c.Low);

        // Check right peak is within ~5% of left peak
        if (Math.Abs(rightPeak - leftPeak) / leftPeak > 0.05)
            return false;

        // Check cup depth is at least ~15% from peaks to bottom
        float cupDepthLeft = (leftPeak - bottom) / leftPeak;
        float cupDepthRight = (rightPeak - bottom) / rightPeak;
        if (cupDepthLeft < 0.15 || cupDepthRight < 0.15)
            return false;

        // Check handle: last 5-10 candles form a downward or sideways consolidation with smaller range than cup depth
        int handleLength = Math.Min(10, count / 4);
        if (handleLength < 5)
            handleLength = 5;
        var handleCandles = candles.Skip(count - handleLength).Take(handleLength).ToList();

        float handleHigh = handleCandles.Max(c => c.High);
        float handleLow = handleCandles.Min(c => c.Low);
        float handleRange = handleHigh - handleLow;

        float cupRange = Math.Max(leftPeak, rightPeak) - bottom;

        // Handle range should be smaller than ~50% of cup depth
        if (handleRange > cupRange * 0.5)
            return false;

        // Check handle is downward or sideways: handle high should be less than or equal to right peak
        if (handleHigh > rightPeak)
            return false;

        // Optional: check handle candles mostly bearish or neutral
        int bearishCount = handleCandles.Count(c => c.Close <= c.Open);
        if (bearishCount < handleLength / 2)
            return false;
